...
#include "pi0_include/definitions.h"
#include "pi0_include/cuts.h"
#include "pi0_include/variables.h"
#include "pi0_include/numu_variables.h"
...
std::ofstream output("output_data.log");
...
void write_reco(const caf::SRSpillProxy* sr, const caf::SRInteractionDLPProxy& j)
{
    output  << CSV(sr->hdr.run) << CSV(sr->hdr.evt) << CSV(sr->hdr.subrun)
	    ...
	    << CSV(vars::pi0_mass(j))
	    << CSV(cuts::all_1muNph_data_cut(j))
